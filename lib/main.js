...
var BYLINE = "Santa Claus is social media savvy! He can browse recent, holiday-themed YouTube videos and Flickr photos. You can, too, by tapping on the dashboard items to toggle themes, and clicking on the map to view content. Zoom in for more detail."
...
function init() {
	...
	// color look-up table
 	...
 	_lutMarkers[PointAtts.RESULT_TYPE_SANTA] = "images/RedLight.png";
 	_lutMarkers[PointAtts.RESULT_TYPE_LIGHTS] = "images/OrangeLight.png";
 	_lutMarkers[PointAtts.RESULT_TYPE_TREE] = "images/GreenLight.png";
 	_lutMarkers[PointAtts.RESULT_TYPE_HOLIDAYS] = "images/BlueLight.png";	
...
function loadLocations(locations) {
	...
	$.each(locations,function(index,value) {
		symbol =  new esri.symbol.PictureMarkerSymbol({
			"url":_lutMarkers[value.attributes.getType()],
			"height":22,
			"width":22,
			"type":"esriPMS"
		});		
...
function postSelection() {
    if(_selected.attributes.getFormat() === "photo"){
		...
		" (<a href='"+_selected.attributes.getSource()+"' target='_blank' class='sourceLink'>click here for source</a>)");
        $("#media").css({
            "margin-left":(($("#pictureFrame").width() - $("#media").width())/2),
            "margin-top":(($("#pictureFrame").height() - $("#media").height())/2)
        });
