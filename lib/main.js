...
var _colorLUT = {}; 	// color look-up table
var _layersLUT = {};
...
function init() {
	...
	// color look-up table
 	...
 	_colorLUT[PointAtts.RESULT_TYPE_SANTA] = COLOR_RED;
 	_colorLUT[PointAtts.RESULT_TYPE_LIGHTS] = COLOR_ORANGE;
 	_colorLUT[PointAtts.RESULT_TYPE_TREE] = COLOR_GREEN;
 	_colorLUT[PointAtts.RESULT_TYPE_HOLIDAYS] = COLOR_BLUE;
	...
	_map.addLayer(_layerSanta);	
 	...
 	_layersLUT[PointAtts.RESULT_TYPE_SANTA] = _layerSanta;
 	_layersLUT[PointAtts.RESULT_TYPE_LIGHTS] = _layerLights;
 	_layersLUT[PointAtts.RESULT_TYPE_TREE] = _layerTree;
 	_layersLUT[PointAtts.RESULT_TYPE_HOLIDAYS] = _layerHolidays;
...
function init2() {
	...
	$(".check").click(function(e){
		var layer = _layersLUT[$(e.target).attr("value")];
	...
	$(".controlCaption").click(function(e){
		...
		var layer = _layersLUT[$(check).attr("value")];
...
function loadLocations() {
	$.each(_locations,function(index,value) {
		value.setSymbol(new esri.symbol.SimpleMarkerSymbol().setColor(_colorLUT[value.attributes.getType()]));
		_layersLUT[value.attributes.getType()].add(value);
	});
}
